...
let rec digits x = match x with
| O -> Nil
| S m' ->
  app
    (let y = Nat.div (S m') (S (S (S (S (S (S (S (S (S (S O)))))))))) in
     digits y) (Cons
    ((Nat.modulo x (S (S (S (S (S (S (S (S (S (S O))))))))))), Nil))
...
let rec list_beq eq x y =
  ...
  | Cons (h1, t1) ->
    (match y with
     ...
     | Cons (h2, t2) ->
       (match eq h1 h2 with
        | True -> list_beq eq t1 t2
        | False -> False))

(** val is_palindrome : nat -> bool **)

let is_palindrome n =
  let ds = digits n in list_beq Nat.eqb ds (rev ds)

(** val max_palindrom : nat -> nat -> nat -> nat **)

let max_palindrom i j max_prod =
  let p = mul i j in
  (match match is_palindrome p with
         | True -> Nat.ltb max_prod p
         | False -> False with
   | True -> p
   | False -> max_prod)
...
let rec find_max_aux_func x =
  ...
  let find_max_aux0 = fun p0 max_prod0 upper_limit0 lower_limit0 ->
    find_max_aux_func (ExistT (p0, (ExistT (max_prod0, (ExistT (upper_limit0,
      lower_limit0))))))
  in
     ...
     (match wildcard' with
      | O ->
        (match n with
         | O -> new_prod
         | S j' ->
           find_max_aux0 (Pair ((fst p), j')) new_prod upper_limit lower_limit)
      | S i' ->
        (match n with
         | O ->
           find_max_aux0 (Pair (i', upper_limit)) new_prod upper_limit
             lower_limit
         | S j' ->
           find_max_aux0 (Pair ((fst p), j')) new_prod upper_limit lower_limit)))
...
let find_max_aux p max_prod upper_limit lower_limit =
  find_max_aux_func (ExistT (p, (ExistT (max_prod, (ExistT (upper_limit,
    lower_limit))))))
